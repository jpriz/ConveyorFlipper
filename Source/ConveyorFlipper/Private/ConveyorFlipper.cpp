...
void FConveyorFlipperModule::StartupModule()
{
	...
	SUBSCRIBE_METHOD(UFGBuildGunStateDismantle::SecondaryFire,[] (CallScope<void(*)(UFGBuildGunState*)>& CallScope2, UFGBuildGunState* UfgBuildGunState)
	{
		...
		{
			//PlayerController->GetHitResultUnderCursor(ECC_GameTraceChannel6, false, HitResult);
			//UE_LOG(LogConveyorFlipperLog, Warning, TEXT("bhit %s"), bHit? TEXT("true") : TEXT("false"));
			{
				...
				if (Actor != NULL){
					...
					if (Actor->GetClass()->IsChildOf(AFGBuildableConveyorBelt::StaticClass()))
					{
						...
						if (!Connection0->IsConnected() && !Connection1->IsConnected())
						{
							...
							{
								NewSplineData.Emplace(FSplinePointData(OldSplineData[i].Location, FlipVector(OldSplineData[i].LeaveTangent), FlipVector(OldSplineData[i].ArriveTangent)));
							}
							NewSplineData.Emplace(FSplinePointData(OldSplineData[0].Location, FlipVector(OldSplineData[0].LeaveTangent), FlipVector(OldSplineData[0].ArriveTangent)));
